...
namespace Framework
{
    ...
    //Lưu ý khai báo delegate này là khai bảo kiểu, nằm trong namespace
    /// <summary>
    ...
    public class Router
    {
        ...
        public void Register(string router, ControllerAction action, string help = "")
        {
            //nếu _routingTable đã chứa route này thì bỏ qua
            if (!_routingTable.ContainsKey(router))
            {
                _routingTable[router] = action;
                _helpTable[router] = help;
            }
        }
        /// <summary>
        /// Phân tích truy vấn và gọi phương thức tương ứng với chuỗi truy vấn
        /// <para>Chuỗi truy vấn bao gồm 2 phần:route và parameter, phân tách bởi kí tự ?</para>
        /// </summary>
        /// <param name="request">Chuỗi truy vấn gồm 2 phần :
        ...
        public void Forward(string request)
        {
            ...
            if (!_routingTable.ContainsKey(req.Route))
                throw new Exception("Command net found !");
